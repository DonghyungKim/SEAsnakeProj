...
% and the gain parameters - by Donghyung Kim, 2016.12.09 (revised)
% [Inputs]
...
function sm_res = GetSM_ClimbingModeWithJoystick(snakeData, fbk, VelFactor,radi, pitch, sm, povs, buttons)
    ...
    elseif (sm.r >= radi.min_ClimbPole)&&(buttons(6))&&(~buttons(5))
        if(isempty(fbk)) ||  ((~isempty(fbk))&&( sm_res.r > radi.init/2)) % -> If feedback is empty, or large radius of helix 
